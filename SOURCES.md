...
61. Better Auth Documentation. “[Security and Origin Protection](https://www.better-auth.com/docs/reference/security).” Accessed 16 September 2026. Used for `Origin` header validation, `trustedOrigins`, Fetch Metadata headers (`Sec-Fetch-Site`/`Sec-Fetch-Mode`), and `SameSite=Lax` cookie protections.
...
- Better Auth general conceptual database documentation may lag version-specific plugin documentation; the pinned generator output (`auth generate` via pinned `auth@1.7.5`) is the authoritative source of truth for Prisma schema definitions.
