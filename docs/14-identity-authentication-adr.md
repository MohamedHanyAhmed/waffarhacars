...
1. **Deterministic Pseudonymous Placeholder with Explicit Secret Key:**
   ...
   const hash = crypto
     ...
     .update(`phone-alias:v1�${canonicalE164}`)
     .digest("hex")
     .slice(0, 32);
...
3. **Domain Separation:**
   - The input to the HMAC is domain-separated with a versioned prefix (`phone-alias:v1�${canonicalE164}`) to prevent cross-protocol collision.
   - The phone rate-limiting/cooldown throttle in PostgreSQL must use either a separate key (`PHONE_THROTTLE_HMAC_KEY`) or a distinct domain prefix (`phone-throttle:v1�${canonicalE164}`) so that throttle hashes cannot be correlated with placeholder email addresses.
...
| :------------------------------- | :------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------- |
| **OTP Brute Force**              | Automated script guessing 6-digit codes                              | - Max 3 attempts per challenge<br>- 180s expiry<br>- WaffarhaCars-owned atomic phone throttle keyed by `hmac_sha256(phone, PHONE_THROTTLE_HMAC_KEY)`<br>- Client-facing IP-based rate limiting on public endpoint (PR 2B) | Failed verification count logged in test suite (PR 2B public HTTP test). Automated production intrusion alerting marked deferred. | Challenge destroyed on 3rd failure; 15-minute phone cooldown enforced by DB throttle (PR 2B test).                          |
| **OTP Replay**                   | Re-submitting previously used OTP                                    | - Atomic single-use challenge consumption in verifier (PR 2B concurrency hard gate)                                                                                                                                       | Re-verification rejection returned to client (PR 2B test)                                                                         | Zero session issued; challenge invalidated immediately (PR 2B test).                                                        |
...
| **Session Hijacking / Fixation** | Stealing cookie via XSS or sniffing                                  | - `HttpOnly`, `Secure`, `SameSite=Lax` cookies<br>- No tokens in `localStorage`<br>- Native database session lookup in PostgreSQL<br>- `cookieCache` disabled (PR 2A)                                                     | Deferred (requires client-side anomaly detection or IP drift tracking)                                                            | Immediate server-side session revocation via Better Auth API (`revokeSession`) against PostgreSQL (PR 2A integration test). |
| **CSRF**                         | Cross-origin request forged by third-party site                      | - `SameSite=Lax` cookies<br>- Better Auth `Origin` validation against `trustedOrigins`<br>- Fetch Metadata (`Sec-Fetch-Site`) checks (PR 2A)                                                                              | Better Auth origin rejection logging (PR 2A test)                                                                                 | Request rejected with HTTP 403 before reaching application handlers (PR 2A test).                                           |
| **Credential Stuffing (Staff)**  | Automated login using leaked password lists                          | - Mandatory TOTP for internal staff (PR 2C)<br>- Client-facing IP rate limiting on sign-in endpoint (PR 2C)<br>- Second-factor lockout via `twoFactor.lockedUntil` after consecutive failed attempts (PR 2C)              | Failed login attempt logging in security audit log (PR 2D test). Production alerting threshold marked deferred.                   | Administrative password reset and second-factor unlock via two-person rule (deferred post-pilot operational runbook).       |
| **Client Role Tampering**        | Submitting `role: "ADMIN"` or unauthorized claims in request payload | - Client request payloads validated with Zod<br>- Caller identity and roles resolved strictly from server-side database session in DAL (PR 2D)                                                                            | Zod validation error logging / DAL unauthorized access logging (PR 2D test)                                                       | Payload stripped; access denied with HTTP 403; event recorded in `security_audit_events` (PR 2D test).                      |
| **Stolen Provider Device**       | Workshop device stolen or accessed while logged in                   | - 12-hour absolute shift limit and 2-hour idle timeout enforced server-side in DAL (PR 2D)                                                                                                                                | Deferred (requires automated device telemetry)                                                                                    | Remote session revocation by Provider Manager or Operations via DAL / Better Auth session revocation (PR 2D test).          |
| **Concurrent OTP Race**          | Submitting valid OTP twice simultaneously                            | - Concurrency test hard gate in PR 2B; requires atomic challenge consumption verifier (PR 2B test)                                                                                                                        | Competing request rejection logged in integration test (PR 2B test)                                                               | Exactly one request issues session; competing request rejected with error (PR 2B test).                                     |
